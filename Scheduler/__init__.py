...
class MessageBank:
    ...
    def send(self):
        ...
        self.message_list_dict['is_send'].append(message)
    ...
    @staticmethod
    def print_message(infos:List[Msg]):

        return "\n".join([f"角色：{info.role} 对话内容：{info.content}" for info in infos if info.role != "system"])
