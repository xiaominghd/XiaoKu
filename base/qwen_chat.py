...
# 上下文规则
1. 你会先看到由"[系统上下文开始]"和"[系统上下文结束]"包裹的特殊消息，其中包含[背景]和[记忆]信息
2. 这些信息是根据当前对话，授权你使用的专属知识。
3. 当主人提问的时候，你可以检查该知识块中的相关内容，但不要显式提及"[记忆]"或"[背景]"标签
4. 真实对话历史从"[系统上下文结束]"之后开始，勿混淆上下文与对话

# 互动原则
1、理解并回应主人当前的话题与情绪，并提供给用户真诚的回答。
2、如果上下文规则中的[记忆]与[背景]与当前聊天相关，请调用这些内容，避免出现输出的幻觉。
...
"""
    ...
    # =============== 通道2：动态上下文专用通道（关键创新！） ===============
    # 构建虚拟对话片段承载上下文（模型最易理解的格式）
    context_block = []
    if context_entries:
        # 虚拟用户提问触发知识加载（模型训练中最常见的模式）
        context_block.append({
            "role": "user",
            "content": "[系统上下文开始]"
        })
        # 虚拟助手回复知识块（符合模型对"assistant提供信息"的认知）
        context_block.append({
            "role": "assistant",
    ...
    messages = [
        ...
        *context_block,       # 专用知识通道（放在system后，真实对话前）
        *history      # 纯净的真实对话
    ]
...
if __name__ == "__main__":
    ...
    response = chat_with_qwen(
        ...
        context_entries=current_context  # 动态注入
    )
    print(response)
